...
<script type="text/javascript">
	...
	$(function() {
		...
		//加上My97Date控件
		$("input[name='stockIncomeBill.vdate']").addClass("Wdate").click(
				function() {
					WdatePicker({
						//可选的选项
						maxDate : new Date(),
				...
				function() {
					...
					$.each($("#edit_table_body tr"), function(index, item) {
						...
						$(item).find("[tag=number]").prop("name",
								"stockIncomeBill.items[" + index + "].number");
						$(item).find("[tag=remark]").prop("name",
								"stockIncomeBill.items[" + index + "].remark");
						$(item).find("[tag=costPrice]").prop(
								"name",
								"stockIncomeBill.items[" + index
										+ "].costPrice");
						$(item).find("[tag=amount]").prop("name",
								"stockIncomeBill.items[" + index + "].amount");
					});
					$("#editForm").submit();
				});
	});
				...
				<table cellspacing="0" cellpadding="0" width="100%" align="left"
					...
					<tr>
						...
						<td class="ui_text_lt"><s:select list="#depots"
								name="stockIncomeBill.depot.id" listKey="id" listValue="name"
						...
						<td><input type="button" value="添加明细"
							...
							<table class="edit_table" cellspacing="0" cellpadding="0"
								...
								<tbody id="edit_table_body">
										...
										<tr>
											...
											<td><s:textfield disabled="true" readonly="true"
												...
												class="searchproduct" /> <s:hidden
													name="stockIncomeBill.items.product.id" tag="pid" /></td>
											<td><s:textfield tag="brand"
													name="stockIncomeBill.items.brand.name"
											...
											<td><s:textfield tag="amount"
													name="stockIncomeBill.items.amount"
													cssClass="ui_input_txt04" readonly="true" /></td>
									...
									<!--更新  -->
									<s:else>
										<s:iterator value="stockIncomeBill.items"></s:iterator>
										<tr>
											<td></td>
											<td><s:textfield disabled="true" readonly="true"
